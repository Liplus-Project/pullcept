...
- 端末の下辺の帯を消した後の実機確認。`.xterm-viewport` を描画から外し、黒い帯とネイティブのスクロールバーの矢印が消えること、端末の使える面積と桁数・行数が変わらないことは、実際のスタイルシートから組んだ採寸用のページで確かめてあるが（#77、下記「診断面」）、WebView2 上の見た目は未確認である。
- 画面の再読み込みに耐えるところの実機確認。再読み込み後に席を持つ行が `❌` を出すこと、繋ぎ直した端末へ出力と入力が戻ること、1 行目の告知、繋ぎ直した端末の開始時刻・起動コマンド・作業ディレクトリが起動時の値であることはいずれも実装済みで、CI の型検査と Rust のコンパイルは通っているが、実機での操作は未確認である。引き金となった観察（2026-08-24）は修正前のものである。
- 走っているアカウントの様子の実機確認。宛先にされてから返すまでの「考え中…」、出力が届いている間の「出力中」、沈黙 1 秒での消失、パネル 16.5rem の中で本人の行の名前が押し出されないことはいずれも実装済みで CI の型検査は通っているが、実機での操作は未確認である（#82、上記「走っているアカウントが何をしているか」）。特に、Claude Code が確認プロンプトで待っている間に出力を止めるかどうかは未計測である——止めないのであれば、待っている行に「出力中」が立ち続ける。「出力中」がその場合でも literal に正しい語であることがこの語を選んだ理由だが、それは待機と稼働を見分けられるという意味ではない。
...
**状態の語は「起動中」のままである。** セッションは実際に走っており、別の語を当てれば走っていることのほうを疑わせる。欠けていたのは語ではなく、出力が切れた事実の告知である。その置き場は端末のバッファ自身——失われた出力が在ったはずの位置——であって、状態の欄ではない。
